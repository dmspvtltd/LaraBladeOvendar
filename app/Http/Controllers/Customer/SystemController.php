...
class SystemController extends Controller
{
    ...
    public static function insert_into_cart_shipping($request)
    {
        ...
        if (isset($shipping) == false) {
            ...
            $cartGroupNameProduct=\App\Model\Cart::where(['customer_id' => auth('customer')->id()])->where('cart_group_id',$request['cart_group_id'])->get()->groupBy('cart_group_id');
            $free_delivery_product=\App\Model\Product::where('free_shipping','1')->get();

            foreach($free_delivery_product as $free_delivery_product){
                foreach ($cartGroupNameProduct as $pro){
                    foreach ($pro as $p){
                        if($p['product_id'] == $free_delivery_product['id']){
