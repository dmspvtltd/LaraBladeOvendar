...
class ProductController extends BaseController
{
    ...
    public function add_new_seller_product()
    {
        $sellers = Seller::where('status','approved')->get();
        $cat = Category::where(['parent_id' => 0])->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        return view('admin-views.product.add-new-seller-product', compact('cat', 'br','sellers'));
    }

    public function store_seller_product(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'seller_id.required' => 'Seller name is required!',
        ...
        if ($request->ajax()) {
            ...
            return redirect()->route('admin.product.add-new-seller-product');
        }
    }
    ...
    public function free_delivery_status(Request $request)
    {
        $product = Product::find($request->id);
        $product->free_shipping = ($product['free_shipping'] == 0 || $product['free_shipping'] == null) ? 1 : 0;
        $product->save();
        $data = $request->status;
        return response()->json($data);
    }
