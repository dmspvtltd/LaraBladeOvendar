...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $topcategories = Category::where('position', 0)->take(20)->get();
        ...
        //Free delivery
        $free_delivery_products = Product::with(['reviews'])->active()
        ...
        return view('web-views.home', compact('featured_products', 'topRated', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories','topcategories','free_delivery_products'));
    }

    public function flash_deals($id)
    {
        $deal = FlashDeal::query()->with(['products.product.reviews'])->where(['id' => $id, 'status' => 1])->whereDate('start_date', '<=', date('Y-m-d'))->whereDate('end_date', '>=', date('Y-m-d'))->first();

        // filter the products if have or not
        if ($deal->products->count()) $deal->products = $deal->products->filter(function ($product) {
            return boolval($product->product);
        });
    ...
    public function all_sellers()
    {
        ...
        return view('web-views.sellers', compact('shops'));
    }
    ...
    public function checkout_complete(Request $request)
    {
        ...
        $orderDetails = OrderDetail::where('order_id', $lastOrder->id)->first();
        
        return view('web-views.checkout-complete',compact('lastOrder','orderDetails'));
    ...
    public function seller_shop(Request $request, $id)
    {
        ...
        $products = Product::whereIn('id', $product_ids)->paginate(50);
    ...
    public function products(Request $request)
    {
        ...
        //free delivery start
        if ($request['data_from'] == 'free_shipping') {
            $query = Product::with(['reviews'])->active()->where('free_shipping', 1);
        }
        //free delivery end
    ...
    public function fashions(Request $request)
    {
        $request['sort_by'] == null ? $request['sort_by'] == 'latest' : $request['sort_by'];
        ...
        if ($request['data_from'] == 'category') {
            ...
            foreach ($products as $product) {
                foreach (json_decode($product['category_ids'], true) as $category) {
                    if (($category['id'] == 13) || ($category['id'] == 17)) {
                        array_push($product_ids, $product['id']);
                    }
                }
            }
    ...
    {
        // $missionVission = BusinessSetting::where('type', 'privacy_policy')->first();
        return view('web-views.mission-vission');
    }

    public function disclaimer()
    {
        // $missionVission = BusinessSetting::where('type', 'privacy_policy')->first();
        return view('web-views.disclaimer');
    }
    ...
    {
        // $missionVission = BusinessSetting::where('type', 'privacy_policy')->first();
        return view('web-views.returnRefund');
    }
