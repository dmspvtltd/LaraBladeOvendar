...
class OrderManager
{
    ...
    // {
    //     $shipping_model = BusinessSetting::where('id','56')->first();
    ...
    //          if ($shipping_model->shipping_method == 'inhouse_shipping') {
    ...
    //              if ($shipping_model->shipping_method == 'sellerwise_shipping') {
    //                  $wallet->delivery_charge_earned += $order['shipping_cost'];
    //              }
    ...
    public static function generate_order($data)
    {
        ...
        $data = [
            ...
            "type" => "text",
            "contacts" => $user->phone."+".$seller->phone,
            "senderid" => "8809601004769",
            "msg" => "NillGiri Order id " . $order_id. " Order Amount " . ( CartManager::cart_grand_total($cart_group_id) - $discount ) ."Tk",
        ];
