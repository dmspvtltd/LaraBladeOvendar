...
class Helpers
{
    ...
    public static function sales_commission($order)
    {
        ...
        foreach ($order->details as $detail) {
            ...
            try {
                $last_category_id = intval(collect(json_decode($detail->product->category_ids))->last()->id ?? false);
            } catch (\Throwable $th) {
                //throw $th;
            }

            if ($last_category_id) $first_category = Category::find($last_category_id);

            if ($first_category) {
                // $item_category = Category::find();
                $price = $detail->price;
                $qty = $detail->qty;
                $tax = 0;
                $discount = $detail->discount; // flat
                $cat_comission = $first_category['commission'];

                $net_detail_amount = ($price - $discount - $tax) * $qty;

                $net_comission = ($net_detail_amount / 100) * $cat_comission;

                return $commission_amount += $net_comission;
            }
        }
