                    ...
                    <div class="row flex-between justify-content-between align-items-center flex-grow-1">
                        ...
                        <div>
                            <a href="{{route('admin.product.add-new')}}" class="btn btn-primary  float-right">
                                ...
                                <span class="text">{{\App\CPU\translate('Add new in-house product')}}</span>
                            </a>
                        </div>
                        <div>
                            <a href="{{route('admin.product.add-new-seller-product')}}" class="btn btn-primary  float-right">
                                <i class="tio-add-circle"></i>
                                <span class="text">{{\App\CPU\translate('Add new seller product')}}</span>
                            </a>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table id="datatable" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            ...
                            <tr>
                                ...
                                <th>Ovendar Mall</th>
                                <th>{{\App\CPU\translate('Free')}} {{\App\CPU\translate('Delivery')}}</th>
                            ...
                            @foreach($pro as $k=>$p)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$p['thumbnail']}}" width="100%" height="100%" alt="">
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <label class="switch">
                                            <input type="checkbox"
                                                   onclick="free_delivery('{{$p['id']}}')" {{$p->free_shipping == 1?'checked':''}}>
                                            <span class="slider round"></span>
                                        </label>
                                    </td>
                                    <td>
...
@push('script')
    ...
    <script>
        ...
        function free_delivery(id) {
            ...
            $.ajax({
                url: "{{route('admin.product.free_delivery-status')}}",
                method: 'POST',
                data: {
                    id: id
                },
                success: function () {
                    toastr.success('{{\App\CPU\translate('Free delivery status updated successfully')}}');
                }
            });
        }
