                    ...
                    <div class="d-sm-flex align-items-sm-center">
                        ...
                        <div class="hs-unfold ml-2">
                            <select class="form-control" name="delivery_man_id" onchange="addDeliveryMan(this.value)">
                                <option value="0">{{ \App\CPU\translate('select') }}
                                    {{ \App\CPU\translate('deliveryman') }}</option>
                    ...
                    <div class="row">
                        ...
                        <script>
                            function approveStatus(status) {
                                ...
                                $.ajax({
                                    url: "{{ route('admin.orders.approved.status') }}",
                                    method: 'POST',
                                    data: {
                                        "id": id,
                                        "order_status": status,
                                    ...
                                    success: function(data) {
                                        toastr.success('Approved Status Change Successfully');
                                    }
                                });
                            }
                        </script>
                        ...
                        <div class="col-6">
                            <div class="hs-unfold float-right pl-2">
                                <div class="dropdown">
                                    <select name="approved_status" class="form-control" onchange="approveStatus(this.value)"
                                        data-id="{{ $order['id'] }}">
                                        <option value="pending"
                                            {{ $order['approved_status'] == 'pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="confirmed"
                                            {{ $order['approved_status'] == 'confirmed' ? 'selected' : '' }}>Confirmed
                                        </option>
                                        <option value="rejected"
                                            {{ $order['approved_status'] == 'rejected' ? 'selected' : '' }}>Rejected
                                        </option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="hs-unfold float-right pr-2">
                                <div class="dropdown">
                                    <select name="payment_status" class="payment_status form-control"
                                        data-id="{{ $order['id'] }}" {{ $order->payment_status == 'paid' ? 'disabled':'' }}>
...
@push('script_2')
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ \App\CPU\Helpers::get_business_settings('map_api_key') }}&v=3.45.8">
    </script>
    <script>
